...
<table id="myTable" class="table table-hover table-centered table-nowrap mb-0 showTableJobs ">
    ...
    <tbody class="sortable">
        @foreach ($demandas as $demanda )
            @if ($demanda['agencia'])
            <tr data-key="{{$demanda->id}}" class="trLink" style="cursor: pointer; background:{{$demanda->count_prazosDaPauta && $loggedUser->id == $demanda->criador_id ? 'rgb(255 219 114 / 85%)' : ''}}" data-href="{{route('Job', ['id' => $demanda->id])}}">
                ...
                <td data-column-index="2">
                    ...
                    @if($demanda->recorrente)
                        <span style="font-size: 10px; font-style: italic">Recorrente</span>
                    @endif
                </td>
                <td data-column-index="3" class="title"> {{ $demanda->titulo }}</td>
                ...
                <td data-date="{{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $demanda->inicio)->format('d/m/Y H:i:s'); }}" data-column-index="5">{{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $demanda->inicio)->format('d/m/Y H:i'); }}</td>
                @if($demanda->recorrente == 1 && $demanda->mostRecentDate != null)
                <td data-date="{{ Carbon\Carbon::createFromFormat('Y-m-d', $demanda->mostRecentDate)->format('d/m/Y') }}" data-column-index="6">
                    {{ Carbon\Carbon::createFromFormat('Y-m-d', $demanda->mostRecentDate)->format('d/m/Y') }}
                    @if(Carbon\Carbon::parse($dataAtualRec)->greaterThan(Carbon\Carbon::parse($demanda->mostRecentDate)))
                        <span class="atrasado">ATRASADO!</span>
                    @endif
                </td>
                @else
                <td data-date="{{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $demanda->final)->format('d/m/Y H:i:s'); }}"  data-column-index="6">
