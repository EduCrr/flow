<table id="myTable" class="table table-hover table-centered table-nowrap mb-0 showTableJobs ">
    ...
    <thead>
        <tr id="sortableHead">
            ...
            <th data-column-index="7">
                <div data-name="marca" data-ordem="{{$ordem ?? 'desc'}}" class="th-content th-coluna">Marca(s)</div>
            </th>
            <th data-column-index="8">
                <div class="th-content">Progresso</div>
            </th>
            <th data-column-index="9">
                <div data-name="agencia" data-ordem="{{$ordem ?? 'desc'}}" class="th-content th-coluna">Agencia</div>
            </th>
    ...
    <tbody class="{{ $sortableEnabled ? '' : 'sortable' }}">
        @foreach ($demandas as $demanda)
            @if ($demanda['agencia'])
                <tr data-key="{{$demanda->id}}" id="sortableColumns" class="trLink" style="cursor: pointer;" data-href="{{route('Job', ['id' => $demanda->id])}}">
                    ...
                    <td data-column-index="10">
                        {{-- @if($demanda->count_questionamentos > 0 || $demanda->count_respostas > 0 || ($loggedUser->id != $demanda->criador_id && $demanda->count_questionamentos_ag > 0))
                        @if(($demanda->count_questionamentos > 0 && $loggedUser->id == $demanda->criador_id) || $demanda->count_respostas > 0 || ($loggedUser->id != $demanda->criador_id && $demanda->count_questionamentos_ag > 0))
                            <span>
