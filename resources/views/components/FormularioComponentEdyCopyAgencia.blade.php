        ...
        <div class="">
            <select id="inputC" name="colaborador" class="form-select select2" required>
                @foreach ($colaboradores as $colaborador )
                    <option @if($colaborador->id == $demanda->criador_id) selected @endif  value="{{ $colaborador->id }}">{{ $colaborador->nome }}</option>
                @endforeach
            </select>
            <div class="invalid-feedback">
...
{{-- <div class="mb-3 row">
    <div class="col-lg-6">
        <label for="inputC" class="col-sm-2 form-label">Novo Colaborador(es)</label>
        <div class="">
            <select class="select2-multiple-colaborador form-control" name="moreColaboradores[]" multiple="multiple"
                id="select2MultipleColaboradores">
                @foreach ($moreColaboradores as $colaborador )
                    @if($demanda->criador->id !== $colaborador->id)
                        <option @if (!empty(old('moreColaboradores')) && in_array($colaborador->id, old('moreColaboradores'))) selected  @endif  value="{{ $colaborador->id }}" data-cor="#111">{{ $colaborador->nome }}</option>
                    @endif
                @endforeach
            </select>
        </div>
...
<div class="mb-3 row">
    <div class="col-lg-6  mo-b-15">
        ...
        <div class="">
            ...
            <input value="{{ Str::contains(request()->url(), 'copiar') ? $dataAtual : old('inicio', $demanda->inicio) }}" class="form-control" name="inicio" type="datetime-local" required
            id="inputI">
            <div class="invalid-feedback">
...
{{-- <div class="mb-3 row">
    <div class="col-lg-6">
        <label for="inputS" class="col-sm-2 form-label flexLoading">Marcas
