                                            ...
                                            <form class="responseAjax" action="{{route('Notification.action')}}" method="POST">
                                                ...
                                                @foreach ($notifications as $item )
                                                    <div class="notifyArea">
                                                        ...
                                                        @endif
                                                        <a href="{{ route('Job', ['id' => $item->demanda_id] + ($item->tipo_referencia ? ['comentario' => preg_replace('/\s+/', '', $item->tipo_referencia)] : [])) }}" class="text-reset notification-item">
                                                            <div class="d-flex align-items-start {{ $item->visualizada == 1 ? 'notifyContent' : '' }}">
                                                                <div class="avatar-xs me-3">
                                                                    <span class="avatar-title bg-primary rounded-circle font-size-16 avatar-notify">
                                                                        ...
                                                                        @elseif($item->tipo === 'criada-coloborador')
                                                                            <i class="mdi mdi-check text-primary"></i>
                                                                        @elseif($item->tipo === 'convidado')
                                                                            <i class="mdi mdi-check text-primary"></i>
                                                                        @endif
                                                                    </span>
                                                                </div>
